...
void jacobi_step_old(params p, double** u_new, double** u_old, double** f, int my_rank, int size) {
    ...
    for (int i = p.xmin; i < p.xmax; i++) {
        ...
        for (int j = p.ymin; j < p.ymax; j++) {
            ...
            int idy = j - p.ymin;
            u_new[idx][idy] =
                0.25 * (u_old[idx - 1][idy] + u_old[idx + 1][idy] + u_old[idx][idy - 1] +
...
void jacobi_step(params p, double** u_new, double** u_old, double** f, int my_rank, int size) {
    ...
    for (int i = p.xmin; i < p.xmax; i++) {
        ...
        for (int j = p.ymin; j < p.ymax; j++) {
            ...
            if (i == p.xmin) {
                ...
                left = u_old[idx - 1][idy];
            }

            if (i == p.xmax - 1) {
                right = fromRight[idy];
            } else {
                right = u_old[idx + 1][idy];
            }
    ...
    if (p.nx != p.ny) {
        std::printf("In function jacobi_step (jacobi.cpp l.26): nx != ny, check jacobi updates\n");
    }
}
