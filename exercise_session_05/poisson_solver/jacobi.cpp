...
double norm_diff(params p, double **mat1, double **mat2) {
    double sum = 0.;
    for (size_t i = 0; i < static_cast<size_t>(p.nx); i++) {
        for (size_t j = 0; j < static_cast<size_t>(p.ny); j++) {
            sum += std::pow(mat1[i][j] - mat2[i][j], 2);
        }
    }
    auto diff = std::sqrt(sum / (p.nx * p.ny));
    return diff;
}

/**
 * @brief      Compute a Jacobi iteration to solve Poisson equation. This
 * function updates u_new and u_old in-place.
 *
 * @param[in]  p      Parameters
